...
foreach ($analytics as $page_key => $pdata) {
    ...
    if ($month_data) {
        ...
        $slug = $pdata['slug'] ?? '';
        // Build display label
        if ($page_key === '__home__') {
            $headline = '🏠 Homepage';
        } elseif ($slug === '' && isset($section_names[$sec])) {
            // Section index page like /ma/ /vc/ etc
            $headline = '📂 ' . $section_names[$sec] . ' (section page)';
        } else {
            $headline = $post_headlines[$page_key] ?? $pdata['label'] ?? $page_key;
        }
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* DAILY BREAKDOWN */
  .daily-breakdown{background:#f9f7f2;border-top:1px solid #eee;padding:16px 14px 16px 34px;}
  ...
  <table class="analytics-table">
    <thead>
      <tr>
        ...
        <th style="width:70px;text-align:right;">Shares</th>
    ...
    foreach ($page_totals as $page_key => $pdata):
        ...
        $row_id = 'row_' . $row_idx++;
    ?>
    <tr class="data-row" onclick="toggleDetail('<?= $row_id ?>')" style="cursor:pointer;">
      <td style="padding:12px 14px;width:32px;">
        <span class="expand-icon" id="<?= $row_id ?>_icon" style="font-size:10px;color:var(--muted);">▶</span>
      </td>
      <td style="padding:12px 14px;">
        <div style="font-size:14px;font-weight:600;line-height:1.3;">
          <?php if ($page_key === '__home__'): ?>
            🏠 Homepage
          <?php else: ?>
            <?php $display_label = $post_headlines[$page_key] ?? $pdata['label'] ?? $page_key; ?>
            <?= e($display_label) ?>
          <?php endif; ?>
        </div>
      </td>
      ...
      <td style="padding:12px 14px;text-align:right;font-family:'Courier New',monospace;font-weight:700;"><?= number_format($pdata['total']) ?></td>
      <td style="padding:12px 14px;text-align:right;font-family:'Courier New',monospace;">
        <?php
          $shares_count = (int)($analytics[$page_key]['shares'] ?? 0);
        ?>
        <?php if($shares_count > 0): ?>
          <span style="color:var(--accent);font-weight:700;">↗ <?= $shares_count ?></span>
        <?php else: ?>
          <span style="color:#ccc;">—</span>
        <?php endif; ?>
      </td>
      <td style="padding:12px 20px 12px 14px;">
        <div style="height:6px;background:<?= $sec_color ?: 'var(--accent)' ?>;border-radius:3px;width:<?= $bar_pct ?>%;"></div>
      </td>
    </tr>
    <tr id="detail_<?= $row_id ?>" style="display:none;">
      <td colspan="5" style="padding:0 14px 16px 48px;background:#f9f7f2;border-bottom:1px solid #ddd;">
        <?php if (empty($days)): ?>
        <p style="font-size:12px;color:var(--muted);padding-top:12px;">No daily data available.</p>
        <?php else: ?>
        <div style="font-family:'Courier New',monospace;font-size:10px;color:var(--muted);letter-spacing:.1em;text-transform:uppercase;padding-top:12px;margin-bottom:10px;">Daily breakdown</div>
...
function toggleDetail(rowId) {
  var detail = document.getElementById('detail_' + rowId);
  var icon   = document.getElementById(rowId + '_icon');
  if (!detail) return;
