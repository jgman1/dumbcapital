...
<!-- Microsoft Clarity -->
<script>(function(c,l,a,r,i,t,y){c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i+"?ref=bwt";y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y);})(window,document,"clarity","script","wum2f6dv0r");</script>
<!-- HilltopAds -->
<script>(function(kmrcz){var d=document,s=d.createElement('script'),l=d.scripts[d.scripts.length-1];s.settings=kmrcz||{};s.src="//deliciouslip.com/bmX.V/s/dKGDlq0mYrWkcy/xezmu9tuZZLUXlKk/PYTAc/wtNzz/Mi0/N/jtUHtLNXz-A/3ZMgzdQ/2uO/QQ";s.async=true;s.referrerPolicy='no-referrer-when-downgrade';l.parentNode.insertBefore(s,l);})({})</script>
...
<body>
<?php
    // HilltopAds handles ad display globally
    global $ticker_posts;
...
function render_ad(): void {
    // HilltopAds script loads globally - no inline ad units needed
}
...
function render_share_buttons(array $post): void {
    ...
    $track_page    = json_encode(post_url($post));
    $track_section = json_encode($post['section'] ?? $post['tag'] ?? '');
    $track_slug    = json_encode($post['slug'] ?? '');
?>
<div class="share-bar">
  ...
  <button class="share-btn" id="copy-btn-<?= substr(md5(post_url($post)),0,6) ?>" onclick="copyLink('<?= e('https://dumbcapital.com'.post_url($post)) ?>', this); trackShare(<?= $track_page ?>, <?= $track_section ?>, <?= $track_slug ?>)">
